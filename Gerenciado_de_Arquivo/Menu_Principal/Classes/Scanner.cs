...
namespace ScannerDemo
{
    class Scanner
    {
        ...
        private int width_pixel = 1260;
        private int height_pixel = 1755;
        public static int color_mode = 2;//1 colorido 2 PB
        ...
        public ImageFile ScanPNG()
        {
            ...
            catch (COMException e)
            {
                ...
                {
                    MessageBox.Show("O scanner está oculpado!");
                    ScanError = true;
                }
                else if(errorCode == 0x80210064)
                {
                    MessageBox.Show("Escaneamento cancelado!");
                    ScanError = true;
                }
                else
                {
        ...
        private static void AdjustScannerSettings(IItem scannnerItem, int scanResolutionDPI, int scanStartLeftPixel, int scanStartTopPixel, int scanWidthPixels, int scanHeightPixels, int brightnessPercents, int contrastPercents, int colorMode)
        {
            ...
            const string WIA_VERTICAL_SCAN_RESOLUTION_DPI = "6149";
